 ...
 * Models spread decay as exponential: spread(t) = spread(0) * e^(-lambda * t)
...
export async function computeSpreadHalfLife(
  ...
  const variance =
    decayRates.reduce((s, v) => s + (v - meanLambda) ** 2, 0) /
    decayRates.length;
  const r2 = 1 - variance / (meanLambda ** 2 + 0.0001);
...
export async function computeCalibration(
  ...
  if (rows.length === 0) {
    return { platform, brierScore: 0, buckets: [], totalResolved: 0 };
  }
